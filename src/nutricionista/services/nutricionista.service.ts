...
export class NutricionistaService {
  ...
  async create(dto: CreateNutricionistaDto): Promise<Nutricionista> {
    const emailExistente = await this.nutricionistaRepository.findOne({
    ...
    const crnExistente = await this.nutricionistaRepository.findOne({
      where: { crn: dto.crn },
    });
  ...
  async update(
    ...
    if (dto.email && dto.email !== nutricionista.email) {
      const emailExistente = await this.nutricionistaRepository.findOne({
        where: { email: dto.email },
      });
    ...
    if (dto.crn && dto.crn !== nutricionista.crn) {
      const crnExistente = await this.nutricionistaRepository.findOne({
        where: { crn: dto.crn },
      });
